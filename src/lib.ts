...
export const isBearBet = (
  ...
  let precalculation:boolean = TREND === "up" ? false : true;
  console.log('initialAmount: ', INITIAL_AMOUNT)
  console.log('currentStep: ', CURRENT_STEP)
  const betAmount = INITIAL_AMOUNT * (2 ** CURRENT_STEP);
  AMOUNT_TO_BET = betAmount + "";
  console.log('betAmount: ', betAmount);
  console.log(AMOUNT_TO_BET)
  CURRENT_STEP ++;
...
let AMOUNT_TO_BET:string;
let MAX_STEPS:number;
let TREND:string;
...
export const startPolling = async (
  ...
  AMOUNT_TO_BET = betAmount || "0.001";
  TREND = trend || "up";
  MAX_STEPS = parseFloat(maxSteps || "1" );
  INITIAL_AMOUNT = parseFloat(initialAmount || "0.001");
  CURRENT_STEP = 0;
  ...
  const poller = async () => {
    ...
    if (
      ...
      console.log('AMOUNT_TO_BET:', AMOUNT_TO_BET);
      if (bearBet) {
        try {
          const tx =
            platform === PLATFORMS.PancakeSwap
          ...
          console.log(blue(`${platform}. Bear Betting Tx Started. Amount: ${AMOUNT_TO_BET}`));
        ...
        try {
          ...
          console.log(blue(`${platform}. Bull Betting Tx Started. Amount: ${AMOUNT_TO_BET}`));
